# 方案文档｜第 2 周最小可交付拆解（week01-text-api）

## 1. 背景与目标

- 当前问题：
  - 第 1 周已经把 `week01-text-api` 做到“本地可跑、接口可用、测试可过、文档可回看”，但还停留在偏基础的工程骨架阶段。
...
- 输入：
  - 当前已有的 `week01-text-api` 代码、README、接口契约、`docs/traces/` 留痕、postmortem 和本地 `.venv` 环境。
- 输出：
...
- 本次必须做：
  ...
  - 为 MiniMax 请求补清晰的超时配置与有限重试策略，避免无限等待或过度重试。
  - 让 `.github/workflows/week01-text-api-pytest.yml` 在 GitHub 上实际跑通至少一次。
...
- 模型或服务：
  - 继续使用 MiniMax Chat Completions 兼容接口，不切换模型供应商。
- 数据存储：
  - 暂时只要求 PostgreSQL 骨架可本机启动验证，不强制接入主请求链路。
- 关键模块：
  - `app/main.py`：请求入口、异常处理、`request_id`、请求级日志
  - `app/minimax_client.py`：超时、有限重试、上游请求日志
  - `tests/`：错误路径、超时/重试、配置边界测试
  - `.github/workflows/week01-text-api-pytest.yml`：最小 CI
