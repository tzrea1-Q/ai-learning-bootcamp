...
3. 构造发给 OpenAI 兼容上游的 payload，并解析上游返回。
...
# 当前项目统一使用的模型名。默认值仍指向当前示例供应商，但可通过环境变量切换。
...
COMMON_ERROR_RESPONSES = {
    ...
    502: {
        "model": ErrorResponse,
        "description": "上游 OpenAI 兼容请求失败或返回结构不符合预期",
    },
}
...
def _normalize_rewrite_result(result: str) -> str:
    ...
    if not lines:
        raise ApiError(
            ...
            message="Upstream returned an empty rewrite response",
